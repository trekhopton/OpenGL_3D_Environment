...
void Camera::update(ShaderObj* shader){
	...
	for(int i = 0; i < shader->uniforms.size(); i++){
		...
		if(shader->uniforms[i] == "viewPos"){
			glUniform3fv(handle, 1, glm::value_ptr(position));
		} 
		else if(shader->uniforms[i] == "view_matrix"){
			glm::mat4 viewMatrix;
			viewMatrix = GetViewMatrix();
			glUniformMatrix4fv(handle, 1, false, glm::value_ptr(viewMatrix));
		}
	}
}
...
void Camera::ProcessMouseMovement(float xoffset, float yoffset)
{
	...
	Yaw   += xoffset;
	Pitch -= yoffset; // make += to invert y axis
